...
import (
	...
	"github.com/klinova/kinara-os/medication-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("medication-service", nil))
	api.HandleFunc("/medications", h.list).Methods(http.MethodGet)
	api.HandleFunc("/medications", h.create).Methods(http.MethodPost)
	api.HandleFunc("/medications/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/medications/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/medications/{id}", h.delete).Methods(http.MethodDelete)
}
