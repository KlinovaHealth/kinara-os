...
import (
	...
	"github.com/klinova/kinara-os/agri-insurance-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("agri-insurance-service", nil))
	api.HandleFunc("/agri-insurances", h.list).Methods(http.MethodGet)
	api.HandleFunc("/agri-insurances", h.create).Methods(http.MethodPost)
	api.HandleFunc("/agri-insurances/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/agri-insurances/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/agri-insurances/{id}", h.delete).Methods(http.MethodDelete)
}
