...
import (
	...
	"github.com/klinova/kinara-os/outcome-tracking-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("outcome-tracking-service", nil))
	api.HandleFunc("/outcome-trackings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/outcome-trackings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/outcome-trackings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/outcome-trackings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/outcome-trackings/{id}", h.delete).Methods(http.MethodDelete)
}
