...
import (
	...
	"github.com/klinova/kinara-os/budget-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("budget-service", nil))
	api.HandleFunc("/budgets", h.list).Methods(http.MethodGet)
	api.HandleFunc("/budgets", h.create).Methods(http.MethodPost)
	api.HandleFunc("/budgets/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/budgets/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/budgets/{id}", h.delete).Methods(http.MethodDelete)
}
