...
import (
	...
	"github.com/klinova/kinara-os/maritime-analytics-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("maritime-analytics-service", nil))
	api.HandleFunc("/maritime-analyticss", h.list).Methods(http.MethodGet)
	api.HandleFunc("/maritime-analyticss", h.create).Methods(http.MethodPost)
	api.HandleFunc("/maritime-analyticss/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/maritime-analyticss/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/maritime-analyticss/{id}", h.delete).Methods(http.MethodDelete)
}
