...
import (
	...
	"github.com/klinova/kinara-os/bill-of-lading-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("bill-of-lading-service", nil))
	api.HandleFunc("/bill-of-ladings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/bill-of-ladings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/bill-of-ladings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/bill-of-ladings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/bill-of-ladings/{id}", h.delete).Methods(http.MethodDelete)
}
