...
import (
	...
	"github.com/klinova/kinara-os/vital-signs-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("vital-signs-service", nil))
	api.HandleFunc("/vital-signss", h.list).Methods(http.MethodGet)
	api.HandleFunc("/vital-signss", h.create).Methods(http.MethodPost)
	api.HandleFunc("/vital-signss/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/vital-signss/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/vital-signss/{id}", h.delete).Methods(http.MethodDelete)
}
