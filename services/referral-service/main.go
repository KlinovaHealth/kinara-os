...
import (
	...
	"github.com/klinova/kinara-os/referral-service/handlers"
...
func main() {
	...
	// ── JWT validator (public key only — auth-service issues tokens) ──────────
	...
	r.HandleFunc("/ready", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ready","service":"referral-service"}`))
	}).Methods(http.MethodGet)
	...
	go func() {
		logger.Info("referral-service starting", "port", port)
