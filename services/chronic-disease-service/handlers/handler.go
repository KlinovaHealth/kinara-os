...
import (
	...
	"github.com/klinova/kinara-os/chronic-disease-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("chronic-disease-service", nil))
	api.HandleFunc("/chronic-diseases", h.list).Methods(http.MethodGet)
	api.HandleFunc("/chronic-diseases", h.create).Methods(http.MethodPost)
	api.HandleFunc("/chronic-diseases/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/chronic-diseases/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/chronic-diseases/{id}", h.delete).Methods(http.MethodDelete)
}
