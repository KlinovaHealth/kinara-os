...
import (
	...
	"github.com/klinova/kinara-os/agri-credit-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("agri-credit-service", nil))
	api.HandleFunc("/agri-credits", h.list).Methods(http.MethodGet)
	api.HandleFunc("/agri-credits", h.create).Methods(http.MethodPost)
	api.HandleFunc("/agri-credits/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/agri-credits/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/agri-credits/{id}", h.delete).Methods(http.MethodDelete)
}
