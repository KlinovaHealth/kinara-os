...
func main() {
	...
	queries := db.New(pool)
	routeHandler := handlers.NewRouteHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"route-service"}`))
	...
	api.Use(pkgauth.RequireTenantScope("route-service", nil))
	routeHandler.RegisterRoutes(api)
	...
	go func() {
		log.Printf("route-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("route-service shutdown complete")
}
