...
import (
	...
	"github.com/klinova/kinara-os/reminders-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("reminders-service", nil))
	api.HandleFunc("/reminderss", h.list).Methods(http.MethodGet)
	api.HandleFunc("/reminderss", h.create).Methods(http.MethodPost)
	api.HandleFunc("/reminderss/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/reminderss/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/reminderss/{id}", h.delete).Methods(http.MethodDelete)
}
