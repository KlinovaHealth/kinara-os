...
func main() {
	...
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		...
		w.Write([]byte(`{"status":"ok","service":"logistics-analytics-service"}`))
	...
	api.Use(pkgauth.RequireTenantScope("logistics-analytics-service", nil))
	...
	go func() {
		log.Printf("logistics-analytics-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("logistics-analytics-service shutdown complete")
}
