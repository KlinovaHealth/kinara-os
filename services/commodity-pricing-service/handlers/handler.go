...
import (
	...
	"github.com/klinova/kinara-os/commodity-pricing-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("commodity-pricing-service", nil))
	api.HandleFunc("/commodity-pricings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/commodity-pricings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/commodity-pricings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/commodity-pricings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/commodity-pricings/{id}", h.delete).Methods(http.MethodDelete)
}
