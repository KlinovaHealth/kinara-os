...
import (
	...
	"github.com/klinova/kinara-os/fertilizer-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("fertilizer-service", nil))
	api.HandleFunc("/fertilizers", h.list).Methods(http.MethodGet)
	api.HandleFunc("/fertilizers", h.create).Methods(http.MethodPost)
	api.HandleFunc("/fertilizers/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/fertilizers/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/fertilizers/{id}", h.delete).Methods(http.MethodDelete)
}
