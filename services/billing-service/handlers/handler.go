...
import (
	...
	"github.com/klinova/kinara-os/billing-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("billing-service", nil))
	api.HandleFunc("/billings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/billings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/billings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/billings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/billings/{id}", h.delete).Methods(http.MethodDelete)
}
