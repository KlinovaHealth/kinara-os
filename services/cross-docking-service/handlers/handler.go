...
import (
	...
	"github.com/klinova/kinara-os/cross-docking-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("cross-docking-service", nil))
	api.HandleFunc("/cross-dockings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/cross-dockings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/cross-dockings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/cross-dockings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/cross-dockings/{id}", h.delete).Methods(http.MethodDelete)
}
