...
import (
	...
	localauth "github.com/klinova/kinara-os/cross-docking-service/auth"
	"github.com/klinova/kinara-os/cross-docking-service/crypto"
	"github.com/klinova/kinara-os/cross-docking-service/db"
	"github.com/klinova/kinara-os/cross-docking-service/handlers"
	"github.com/klinova/kinara-os/cross-docking-service/middleware"
)
...
func TestNoAuth_Returns401(t *testing.T) {
	...
	resp, err := http.Post(srv.URL+"/api/v1/cross-dockings", "application/json", bytes.NewBufferString(`{}`))
	if err != nil {
		t.Fatal(err)
	}
	resp.Body.Close()
...
func TestInvalidToken_Returns401(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/api/v1/cross-dockings", bytes.NewBufferString(`{}`))
...
func TestCreate_StoresCiphertext(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/api/v1/cross-dockings", bytes.NewBufferString(body))
...
func TestCrossTenantList(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, srv.URL+"/api/v1/cross-dockings", nil)
...
func TestCrossTenantGet(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, srv.URL+"/api/v1/cross-dockings/"+idB.String(), nil)
	req.Header.Set("Authorization", "Bearer "+mintToken(t, "klinova", tenantA))
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
