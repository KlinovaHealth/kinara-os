...
import (
	...
	"github.com/klinova/kinara-os/user-management-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("user-management-service", nil))
	api.HandleFunc("/user-managements", h.list).Methods(http.MethodGet)
	api.HandleFunc("/user-managements", h.create).Methods(http.MethodPost)
	api.HandleFunc("/user-managements/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/user-managements/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/user-managements/{id}", h.delete).Methods(http.MethodDelete)
}
