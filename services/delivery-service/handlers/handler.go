...
import (
	...
	"github.com/klinova/kinara-os/delivery-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("delivery-service", nil))
	api.HandleFunc("/deliverys", h.list).Methods(http.MethodGet)
	api.HandleFunc("/deliverys", h.create).Methods(http.MethodPost)
	api.HandleFunc("/deliverys/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/deliverys/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/deliverys/{id}", h.delete).Methods(http.MethodDelete)
}
