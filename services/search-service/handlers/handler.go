...
import (
	...
	"github.com/klinova/kinara-os/search-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("search-service", nil))
	api.HandleFunc("/searchs", h.list).Methods(http.MethodGet)
	api.HandleFunc("/searchs", h.create).Methods(http.MethodPost)
	api.HandleFunc("/searchs/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/searchs/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/searchs/{id}", h.delete).Methods(http.MethodDelete)
}
