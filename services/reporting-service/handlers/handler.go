...
import (
	...
	"github.com/klinova/kinara-os/reporting-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("reporting-service", nil))
	api.HandleFunc("/reportings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/reportings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/reportings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/reportings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/reportings/{id}", h.delete).Methods(http.MethodDelete)
}
