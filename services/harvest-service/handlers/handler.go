...
import (
	...
	"github.com/klinova/kinara-os/harvest-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("harvest-service", nil))
	api.HandleFunc("/harvests", h.list).Methods(http.MethodGet)
	api.HandleFunc("/harvests", h.create).Methods(http.MethodPost)
	api.HandleFunc("/harvests/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/harvests/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/harvests/{id}", h.delete).Methods(http.MethodDelete)
}
