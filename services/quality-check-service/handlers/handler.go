...
import (
	...
	"github.com/klinova/kinara-os/quality-check-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("quality-check-service", nil))
	api.HandleFunc("/quality-checks", h.list).Methods(http.MethodGet)
	api.HandleFunc("/quality-checks", h.create).Methods(http.MethodPost)
	api.HandleFunc("/quality-checks/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/quality-checks/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/quality-checks/{id}", h.delete).Methods(http.MethodDelete)
}
