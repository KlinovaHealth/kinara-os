...
func main() {
	...
	queries := db.New(pool)
	marketHandler := handlers.NewMarketHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"market-service"}`))
	...
	api.Use(pkgauth.RequireTenantScope("market-service", nil))
	marketHandler.RegisterRoutes(api)
	...
	go func() {
		log.Printf("market-service listening on :%s (TLS)", port)
