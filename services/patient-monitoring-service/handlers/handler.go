...
import (
	...
	"github.com/klinova/kinara-os/patient-monitoring-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("patient-monitoring-service", nil))
	api.HandleFunc("/patient-monitorings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/patient-monitorings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/patient-monitorings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/patient-monitorings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/patient-monitorings/{id}", h.delete).Methods(http.MethodDelete)
}
