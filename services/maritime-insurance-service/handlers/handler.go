...
import (
	...
	"github.com/klinova/kinara-os/maritime-insurance-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("maritime-insurance-service", nil))
	api.HandleFunc("/maritime-insurances", h.list).Methods(http.MethodGet)
	api.HandleFunc("/maritime-insurances", h.create).Methods(http.MethodPost)
	api.HandleFunc("/maritime-insurances/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/maritime-insurances/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/maritime-insurances/{id}", h.delete).Methods(http.MethodDelete)
}
