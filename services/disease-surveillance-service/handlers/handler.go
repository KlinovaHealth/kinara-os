...
import (
	...
	"github.com/klinova/kinara-os/disease-surveillance-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("disease-surveillance-service", nil))
	api.HandleFunc("/disease-surveillances", h.list).Methods(http.MethodGet)
	api.HandleFunc("/disease-surveillances", h.create).Methods(http.MethodPost)
	api.HandleFunc("/disease-surveillances/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/disease-surveillances/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/disease-surveillances/{id}", h.delete).Methods(http.MethodDelete)
}
