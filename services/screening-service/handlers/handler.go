...
import (
	...
	"github.com/klinova/kinara-os/screening-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("screening-service", nil))
	api.HandleFunc("/screenings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/screenings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/screenings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/screenings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/screenings/{id}", h.delete).Methods(http.MethodDelete)
}
