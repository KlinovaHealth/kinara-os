...
import (
	...
	"github.com/klinova/kinara-os/maritime-agent-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("maritime-agent-service", nil))
	api.HandleFunc("/maritime-agents", h.list).Methods(http.MethodGet)
	api.HandleFunc("/maritime-agents", h.create).Methods(http.MethodPost)
	api.HandleFunc("/maritime-agents/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/maritime-agents/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/maritime-agents/{id}", h.delete).Methods(http.MethodDelete)
}
