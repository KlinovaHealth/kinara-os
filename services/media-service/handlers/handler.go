...
import (
	...
	"github.com/klinova/kinara-os/media-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("media-service", nil))
	api.HandleFunc("/medias", h.list).Methods(http.MethodGet)
	api.HandleFunc("/medias", h.create).Methods(http.MethodPost)
	api.HandleFunc("/medias/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/medias/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/medias/{id}", h.delete).Methods(http.MethodDelete)
}
