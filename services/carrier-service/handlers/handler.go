...
import (
	...
	"github.com/klinova/kinara-os/carrier-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("carrier-service", nil))
	api.HandleFunc("/carriers", h.list).Methods(http.MethodGet)
	api.HandleFunc("/carriers", h.create).Methods(http.MethodPost)
	api.HandleFunc("/carriers/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/carriers/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/carriers/{id}", h.delete).Methods(http.MethodDelete)
}
