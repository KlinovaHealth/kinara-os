...
import (
	...
	"github.com/klinova/kinara-os/nutrition-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("nutrition-service", nil))
	api.HandleFunc("/nutritions", h.list).Methods(http.MethodGet)
	api.HandleFunc("/nutritions", h.create).Methods(http.MethodPost)
	api.HandleFunc("/nutritions/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/nutritions/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/nutritions/{id}", h.delete).Methods(http.MethodDelete)
}
