...
import (
	...
	"github.com/klinova/kinara-os/farmer-service/handlers"
...
func main() {
	...
	if err != nil || len(keyBytes) != 32 {
		logger.Error("ENCRYPTION_KEY must be a 64-character hex string")
		os.Exit(1)
	}
	...
	r.HandleFunc("/ready", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ready","service":"farmer-service"}`))
	}).Methods(http.MethodGet)
	...
	go func() {
		logger.Info("farmer-service starting", "port", port)
	...
	logger.Info("farmer-service stopped")
}
