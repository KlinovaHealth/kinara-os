...
import (
	...
	"github.com/klinova/kinara-os/port-authority-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("port-authority-service", nil))
	api.HandleFunc("/port-authoritys", h.list).Methods(http.MethodGet)
	api.HandleFunc("/port-authoritys", h.create).Methods(http.MethodPost)
	api.HandleFunc("/port-authoritys/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/port-authoritys/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/port-authoritys/{id}", h.delete).Methods(http.MethodDelete)
}
