...
func main() {
	...
	queries := db.New(pool)
	complianceHandler := handlers.NewComplianceHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"compliance-service"}`))
	...
	go func() {
		log.Printf("compliance-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("compliance-service shutdown complete")
}
