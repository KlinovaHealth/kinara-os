...
import (
	...
	"github.com/klinova/kinara-os/allergy-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("allergy-service", nil))
	api.HandleFunc("/allergys", h.list).Methods(http.MethodGet)
	api.HandleFunc("/allergys", h.create).Methods(http.MethodPost)
	api.HandleFunc("/allergys/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/allergys/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/allergys/{id}", h.delete).Methods(http.MethodDelete)
}
