...
import (
	...
	localauth "github.com/klinova/kinara-os/allergy-service/auth"
	"github.com/klinova/kinara-os/allergy-service/crypto"
	"github.com/klinova/kinara-os/allergy-service/db"
	"github.com/klinova/kinara-os/allergy-service/handlers"
	"github.com/klinova/kinara-os/allergy-service/middleware"
)
...
func TestNoAuth_Returns401(t *testing.T) {
	...
	resp, err := http.Post(srv.URL+"/api/v1/allergys", "application/json", bytes.NewBufferString(`{}`))
	if err != nil {
		t.Fatal(err)
	}
	resp.Body.Close()
...
func TestInvalidToken_Returns401(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/api/v1/allergys", bytes.NewBufferString(`{}`))
...
func TestCreate_StoresCiphertext(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/api/v1/allergys", bytes.NewBufferString(body))
...
func TestCrossTenantList(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, srv.URL+"/api/v1/allergys", nil)
...
func TestCrossTenantGet(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, srv.URL+"/api/v1/allergys/"+idB.String(), nil)
	req.Header.Set("Authorization", "Bearer "+mintToken(t, "klinova", tenantA))
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
