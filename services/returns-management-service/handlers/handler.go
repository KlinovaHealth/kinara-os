...
import (
	...
	"github.com/klinova/kinara-os/returns-management-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("returns-management-service", nil))
	api.HandleFunc("/returns-managements", h.list).Methods(http.MethodGet)
	api.HandleFunc("/returns-managements", h.create).Methods(http.MethodPost)
	api.HandleFunc("/returns-managements/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/returns-managements/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/returns-managements/{id}", h.delete).Methods(http.MethodDelete)
}
