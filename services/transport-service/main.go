...
func main() {
	...
	queries := db.New(pool)
	transportHandler := handlers.NewTransportHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"transport-service"}`))
	...
	api.Use(pkgauth.RequireTenantScope("transport-service", nil))
	...
	go func() {
		log.Printf("transport-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("transport-service shutdown complete")
}
