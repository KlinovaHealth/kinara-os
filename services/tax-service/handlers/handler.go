...
import (
	...
	"github.com/klinova/kinara-os/tax-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("tax-service", nil))
	api.HandleFunc("/taxs", h.list).Methods(http.MethodGet)
	api.HandleFunc("/taxs", h.create).Methods(http.MethodPost)
	api.HandleFunc("/taxs/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/taxs/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/taxs/{id}", h.delete).Methods(http.MethodDelete)
}
