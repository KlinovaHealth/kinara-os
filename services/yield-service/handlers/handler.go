...
import (
	...
	"github.com/klinova/kinara-os/yield-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("yield-service", nil))
	api.HandleFunc("/yields", h.list).Methods(http.MethodGet)
	api.HandleFunc("/yields", h.create).Methods(http.MethodPost)
	api.HandleFunc("/yields/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/yields/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/yields/{id}", h.delete).Methods(http.MethodDelete)
}
