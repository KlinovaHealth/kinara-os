...
import (
	...
	"github.com/klinova/kinara-os/symptom-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("symptom-service", nil))
	api.HandleFunc("/symptoms", h.list).Methods(http.MethodGet)
	api.HandleFunc("/symptoms", h.create).Methods(http.MethodPost)
	api.HandleFunc("/symptoms/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/symptoms/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/symptoms/{id}", h.delete).Methods(http.MethodDelete)
}
