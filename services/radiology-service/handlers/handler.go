...
import (
	...
	"github.com/klinova/kinara-os/radiology-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("radiology-service", nil))
	api.HandleFunc("/radiologys", h.list).Methods(http.MethodGet)
	api.HandleFunc("/radiologys", h.create).Methods(http.MethodPost)
	api.HandleFunc("/radiologys/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/radiologys/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/radiologys/{id}", h.delete).Methods(http.MethodDelete)
}
