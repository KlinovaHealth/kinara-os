...
import (
	...
	"github.com/klinova/kinara-os/subsidy-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("subsidy-service", nil))
	api.HandleFunc("/subsidys", h.list).Methods(http.MethodGet)
	api.HandleFunc("/subsidys", h.create).Methods(http.MethodPost)
	api.HandleFunc("/subsidys/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/subsidys/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/subsidys/{id}", h.delete).Methods(http.MethodDelete)
}
