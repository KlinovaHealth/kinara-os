...
import (
	...
	"github.com/klinova/kinara-os/pest-management-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("pest-management-service", nil))
	api.HandleFunc("/pest-managements", h.list).Methods(http.MethodGet)
	api.HandleFunc("/pest-managements", h.create).Methods(http.MethodPost)
	api.HandleFunc("/pest-managements/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/pest-managements/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/pest-managements/{id}", h.delete).Methods(http.MethodDelete)
}
