...
import (
	...
	"github.com/klinova/kinara-os/logistics-reporting-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("logistics-reporting-service", nil))
	api.HandleFunc("/logistics-reportings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/logistics-reportings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/logistics-reportings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/logistics-reportings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/logistics-reportings/{id}", h.delete).Methods(http.MethodDelete)
}
