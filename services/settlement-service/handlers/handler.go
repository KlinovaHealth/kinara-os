...
import (
	...
	"github.com/klinova/kinara-os/settlement-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("settlement-service", nil))
	api.HandleFunc("/settlements", h.list).Methods(http.MethodGet)
	api.HandleFunc("/settlements", h.create).Methods(http.MethodPost)
	api.HandleFunc("/settlements/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/settlements/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/settlements/{id}", h.delete).Methods(http.MethodDelete)
}
