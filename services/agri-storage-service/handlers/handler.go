...
import (
	...
	"github.com/klinova/kinara-os/agri-storage-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("agri-storage-service", nil))
	api.HandleFunc("/agri-storages", h.list).Methods(http.MethodGet)
	api.HandleFunc("/agri-storages", h.create).Methods(http.MethodPost)
	api.HandleFunc("/agri-storages/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/agri-storages/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/agri-storages/{id}", h.delete).Methods(http.MethodDelete)
}
