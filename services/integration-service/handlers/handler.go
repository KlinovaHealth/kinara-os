...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("integration-service", nil))
	api.HandleFunc("/integrations", h.list).Methods(http.MethodGet)
	api.HandleFunc("/integrations", h.create).Methods(http.MethodPost)
	api.HandleFunc("/integrations/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/integrations/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/integrations/{id}", h.delete).Methods(http.MethodDelete)
}
