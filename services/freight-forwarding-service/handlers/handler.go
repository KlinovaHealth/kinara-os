...
import (
	...
	"github.com/klinova/kinara-os/freight-forwarding-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("freight-forwarding-service", nil))
	api.HandleFunc("/freight-forwardings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/freight-forwardings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/freight-forwardings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/freight-forwardings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/freight-forwardings/{id}", h.delete).Methods(http.MethodDelete)
}
