...
import (
	...
	"github.com/klinova/kinara-os/crop-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("crop-service", nil))
	api.HandleFunc("/crops", h.list).Methods(http.MethodGet)
	api.HandleFunc("/crops", h.create).Methods(http.MethodPost)
	api.HandleFunc("/crops/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/crops/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/crops/{id}", h.delete).Methods(http.MethodDelete)
}
