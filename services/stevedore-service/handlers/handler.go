...
import (
	...
	"github.com/klinova/kinara-os/stevedore-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("stevedore-service", nil))
	api.HandleFunc("/stevedores", h.list).Methods(http.MethodGet)
	api.HandleFunc("/stevedores", h.create).Methods(http.MethodPost)
	api.HandleFunc("/stevedores/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/stevedores/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/stevedores/{id}", h.delete).Methods(http.MethodDelete)
}
