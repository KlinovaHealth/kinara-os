...
import (
	...
	"github.com/klinova/kinara-os/toll-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("toll-service", nil))
	api.HandleFunc("/tolls", h.list).Methods(http.MethodGet)
	api.HandleFunc("/tolls", h.create).Methods(http.MethodPost)
	api.HandleFunc("/tolls/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/tolls/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/tolls/{id}", h.delete).Methods(http.MethodDelete)
}
