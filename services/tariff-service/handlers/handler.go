...
import (
	...
	"github.com/klinova/kinara-os/tariff-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("tariff-service", nil))
	api.HandleFunc("/tariffs", h.list).Methods(http.MethodGet)
	api.HandleFunc("/tariffs", h.create).Methods(http.MethodPost)
	api.HandleFunc("/tariffs/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/tariffs/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/tariffs/{id}", h.delete).Methods(http.MethodDelete)
}
