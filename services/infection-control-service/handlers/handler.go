...
import (
	...
	"github.com/klinova/kinara-os/infection-control-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("infection-control-service", nil))
	api.HandleFunc("/infection-controls", h.list).Methods(http.MethodGet)
	api.HandleFunc("/infection-controls", h.create).Methods(http.MethodPost)
	api.HandleFunc("/infection-controls/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/infection-controls/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/infection-controls/{id}", h.delete).Methods(http.MethodDelete)
}
