...
func main() {
	...
	r.HandleFunc("/ready", readyCheck(pool)).Methods(http.MethodGet)
	...
	go func() {
		logger.Info("clinical-service starting", "port", port)
	...
	logger.Info("clinical-service stopped")
}
...
func readyCheck(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ready","service":"clinical-service"}`))
	}
}
