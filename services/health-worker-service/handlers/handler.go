...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("health-worker-service", nil))
	api.HandleFunc("/health-workers", h.list).Methods(http.MethodGet)
	api.HandleFunc("/health-workers", h.create).Methods(http.MethodPost)
	api.HandleFunc("/health-workers/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/health-workers/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/health-workers/{id}", h.delete).Methods(http.MethodDelete)
}
