...
import (
	...
	"github.com/klinova/kinara-os/surgery-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("surgery-service", nil))
	api.HandleFunc("/surgerys", h.list).Methods(http.MethodGet)
	api.HandleFunc("/surgerys", h.create).Methods(http.MethodPost)
	api.HandleFunc("/surgerys/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/surgerys/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/surgerys/{id}", h.delete).Methods(http.MethodDelete)
}
