...
func main() {
	...
	v, err := auth.NewValidator(os.Getenv("JWT_PUBLIC_KEY"))
	if err != nil {
	...
	r := mux.NewRouter()
	r.Use(middleware.JWT(v))
	r.Use(pkgauth.RequireTenantScope("immunization-service", nil))
	r.Use(middleware.Logging(logger))
	h.Register(r)
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"immunization-service"}`))
	})
	...
	logger.Info("immunization-service starting", "port", port)
