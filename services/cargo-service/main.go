...
func main() {
	...
	queries := db.New(pool)
	cargoHandler := handlers.NewCargoHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"cargo-service"}`))
	...
	go func() {
		log.Printf("cargo-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("cargo-service shutdown complete")
}
