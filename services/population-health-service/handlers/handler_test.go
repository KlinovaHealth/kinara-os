...
import (
	...
	localauth "github.com/klinova/kinara-os/population-health-service/auth"
	"github.com/klinova/kinara-os/population-health-service/crypto"
	"github.com/klinova/kinara-os/population-health-service/db"
	"github.com/klinova/kinara-os/population-health-service/handlers"
	"github.com/klinova/kinara-os/population-health-service/middleware"
)
...
func TestNoAuth_Returns401(t *testing.T) {
	...
	resp, err := http.Post(srv.URL+"/api/v1/population-healths", "application/json", bytes.NewBufferString(`{}`))
	if err != nil {
		t.Fatal(err)
	}
	resp.Body.Close()
...
func TestInvalidToken_Returns401(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/api/v1/population-healths", bytes.NewBufferString(`{}`))
