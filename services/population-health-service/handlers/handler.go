...
import (
	...
	"github.com/klinova/kinara-os/population-health-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("population-health-service", nil))
	api.HandleFunc("/population-healths", h.list).Methods(http.MethodGet)
	api.HandleFunc("/population-healths", h.create).Methods(http.MethodPost)
	api.HandleFunc("/population-healths/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/population-healths/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/population-healths/{id}", h.delete).Methods(http.MethodDelete)
}
