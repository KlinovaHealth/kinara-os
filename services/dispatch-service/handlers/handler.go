...
import (
	...
	"github.com/klinova/kinara-os/dispatch-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("dispatch-service", nil))
	api.HandleFunc("/dispatchs", h.list).Methods(http.MethodGet)
	api.HandleFunc("/dispatchs", h.create).Methods(http.MethodPost)
	api.HandleFunc("/dispatchs/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/dispatchs/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/dispatchs/{id}", h.delete).Methods(http.MethodDelete)
}
