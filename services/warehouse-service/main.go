...
func main() {
	...
	api.Use(pkgauth.RequireTenantScope("warehouse-service", nil))
	...
	go func() {
		log.Printf("warehouse-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("warehouse-service shutdown complete")
}
