...
import (
	...
	"github.com/klinova/kinara-os/seed-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("seed-service", nil))
	api.HandleFunc("/seeds", h.list).Methods(http.MethodGet)
	api.HandleFunc("/seeds", h.create).Methods(http.MethodPost)
	api.HandleFunc("/seeds/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/seeds/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/seeds/{id}", h.delete).Methods(http.MethodDelete)
}
