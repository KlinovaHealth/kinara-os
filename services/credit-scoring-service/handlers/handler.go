...
import (
	...
	"github.com/klinova/kinara-os/credit-scoring-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("credit-scoring-service", nil))
	api.HandleFunc("/credit-scorings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/credit-scorings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/credit-scorings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/credit-scorings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/credit-scorings/{id}", h.delete).Methods(http.MethodDelete)
}
