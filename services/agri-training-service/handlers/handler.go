...
import (
	...
	"github.com/klinova/kinara-os/agri-training-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("agri-training-service", nil))
	api.HandleFunc("/agri-trainings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/agri-trainings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/agri-trainings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/agri-trainings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/agri-trainings/{id}", h.delete).Methods(http.MethodDelete)
}
