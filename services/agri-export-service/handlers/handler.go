...
import (
	...
	"github.com/klinova/kinara-os/agri-export-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("agri-export-service", nil))
	api.HandleFunc("/agri-exports", h.list).Methods(http.MethodGet)
	api.HandleFunc("/agri-exports", h.create).Methods(http.MethodPost)
	api.HandleFunc("/agri-exports/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/agri-exports/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/agri-exports/{id}", h.delete).Methods(http.MethodDelete)
}
