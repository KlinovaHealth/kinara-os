...
import (
	...
	"github.com/klinova/kinara-os/child-health-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("child-health-service", nil))
	api.HandleFunc("/child-healths", h.list).Methods(http.MethodGet)
	api.HandleFunc("/child-healths", h.create).Methods(http.MethodPost)
	api.HandleFunc("/child-healths/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/child-healths/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/child-healths/{id}", h.delete).Methods(http.MethodDelete)
}
