...
import (
	...
	"github.com/klinova/kinara-os/logistics-compliance-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("logistics-compliance-service", nil))
	api.HandleFunc("/logistics-compliances", h.list).Methods(http.MethodGet)
	api.HandleFunc("/logistics-compliances", h.create).Methods(http.MethodPost)
	api.HandleFunc("/logistics-compliances/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/logistics-compliances/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/logistics-compliances/{id}", h.delete).Methods(http.MethodDelete)
}
