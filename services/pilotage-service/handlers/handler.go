...
import (
	...
	"github.com/klinova/kinara-os/pilotage-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("pilotage-service", nil))
	api.HandleFunc("/pilotages", h.list).Methods(http.MethodGet)
	api.HandleFunc("/pilotages", h.create).Methods(http.MethodPost)
	api.HandleFunc("/pilotages/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/pilotages/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/pilotages/{id}", h.delete).Methods(http.MethodDelete)
}
