...
import (
	...
	"github.com/klinova/kinara-os/farm-plot-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("farm-plot-service", nil))
	api.HandleFunc("/farm-plots", h.list).Methods(http.MethodGet)
	api.HandleFunc("/farm-plots", h.create).Methods(http.MethodPost)
	api.HandleFunc("/farm-plots/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/farm-plots/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/farm-plots/{id}", h.delete).Methods(http.MethodDelete)
}
