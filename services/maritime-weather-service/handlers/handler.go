...
import (
	...
	"github.com/klinova/kinara-os/maritime-weather-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("maritime-weather-service", nil))
	api.HandleFunc("/maritime-weathers", h.list).Methods(http.MethodGet)
	api.HandleFunc("/maritime-weathers", h.create).Methods(http.MethodPost)
	api.HandleFunc("/maritime-weathers/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/maritime-weathers/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/maritime-weathers/{id}", h.delete).Methods(http.MethodDelete)
}
