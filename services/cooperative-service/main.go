...
func main() {
	...
	queries := db.New(pool)
	coopHandler := handlers.NewCoopHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"cooperative-service"}`))
	...
	api.Use(pkgauth.RequireTenantScope("cooperative-service", nil))
	coopHandler.RegisterRoutes(api)
	...
	go func() {
		log.Printf("cooperative-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("cooperative-service shutdown complete")
}
