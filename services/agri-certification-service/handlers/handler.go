...
import (
	...
	"github.com/klinova/kinara-os/agri-certification-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("agri-certification-service", nil))
	api.HandleFunc("/agri-certifications", h.list).Methods(http.MethodGet)
	api.HandleFunc("/agri-certifications", h.create).Methods(http.MethodPost)
	api.HandleFunc("/agri-certifications/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/agri-certifications/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/agri-certifications/{id}", h.delete).Methods(http.MethodDelete)
}
