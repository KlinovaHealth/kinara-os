...
import (
	...
	"github.com/klinova/kinara-os/medical-history-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("medical-history-service", nil))
	api.HandleFunc("/medical-historys", h.list).Methods(http.MethodGet)
	api.HandleFunc("/medical-historys", h.create).Methods(http.MethodPost)
	api.HandleFunc("/medical-historys/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/medical-historys/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/medical-historys/{id}", h.delete).Methods(http.MethodDelete)
}
