...
import (
	...
	"github.com/klinova/kinara-os/sorting-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("sorting-service", nil))
	api.HandleFunc("/sortings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/sortings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/sortings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/sortings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/sortings/{id}", h.delete).Methods(http.MethodDelete)
}
