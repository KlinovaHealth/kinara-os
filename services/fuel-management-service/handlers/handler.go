...
import (
	...
	"github.com/klinova/kinara-os/fuel-management-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("fuel-management-service", nil))
	api.HandleFunc("/fuel-managements", h.list).Methods(http.MethodGet)
	api.HandleFunc("/fuel-managements", h.create).Methods(http.MethodPost)
	api.HandleFunc("/fuel-managements/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/fuel-managements/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/fuel-managements/{id}", h.delete).Methods(http.MethodDelete)
}
