...
import (
	...
	"github.com/klinova/kinara-os/marine-inspection-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("marine-inspection-service", nil))
	api.HandleFunc("/marine-inspections", h.list).Methods(http.MethodGet)
	api.HandleFunc("/marine-inspections", h.create).Methods(http.MethodPost)
	api.HandleFunc("/marine-inspections/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/marine-inspections/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/marine-inspections/{id}", h.delete).Methods(http.MethodDelete)
}
