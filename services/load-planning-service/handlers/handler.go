...
import (
	...
	"github.com/klinova/kinara-os/load-planning-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("load-planning-service", nil))
	api.HandleFunc("/load-plannings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/load-plannings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/load-plannings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/load-plannings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/load-plannings/{id}", h.delete).Methods(http.MethodDelete)
}
