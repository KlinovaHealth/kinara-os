...
import (
	...
	"github.com/klinova/kinara-os/vehicle-maintenance-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("vehicle-maintenance-service", nil))
	api.HandleFunc("/vehicle-maintenances", h.list).Methods(http.MethodGet)
	api.HandleFunc("/vehicle-maintenances", h.create).Methods(http.MethodPost)
	api.HandleFunc("/vehicle-maintenances/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/vehicle-maintenances/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/vehicle-maintenances/{id}", h.delete).Methods(http.MethodDelete)
}
