...
import (
	...
	"github.com/klinova/kinara-os/manifest-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("manifest-service", nil))
	api.HandleFunc("/manifests", h.list).Methods(http.MethodGet)
	api.HandleFunc("/manifests", h.create).Methods(http.MethodPost)
	api.HandleFunc("/manifests/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/manifests/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/manifests/{id}", h.delete).Methods(http.MethodDelete)
}
