...
import (
	...
	"github.com/klinova/kinara-os/translation-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("translation-service", nil))
	api.HandleFunc("/translations", h.list).Methods(http.MethodGet)
	api.HandleFunc("/translations", h.create).Methods(http.MethodPost)
	api.HandleFunc("/translations/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/translations/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/translations/{id}", h.delete).Methods(http.MethodDelete)
}
