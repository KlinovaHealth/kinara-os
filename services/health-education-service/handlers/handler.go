...
import (
	...
	"github.com/klinova/kinara-os/health-education-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("health-education-service", nil))
	api.HandleFunc("/health-educations", h.list).Methods(http.MethodGet)
	api.HandleFunc("/health-educations", h.create).Methods(http.MethodPost)
	api.HandleFunc("/health-educations/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/health-educations/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/health-educations/{id}", h.delete).Methods(http.MethodDelete)
}
