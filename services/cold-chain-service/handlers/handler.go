...
import (
	...
	"github.com/klinova/kinara-os/cold-chain-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("cold-chain-service", nil))
	api.HandleFunc("/cold-chains", h.list).Methods(http.MethodGet)
	api.HandleFunc("/cold-chains", h.create).Methods(http.MethodPost)
	api.HandleFunc("/cold-chains/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/cold-chains/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/cold-chains/{id}", h.delete).Methods(http.MethodDelete)
}
