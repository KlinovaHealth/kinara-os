...
import (
	...
	"github.com/klinova/kinara-os/patient-engagement-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("patient-engagement-service", nil))
	api.HandleFunc("/patient-engagements", h.list).Methods(http.MethodGet)
	api.HandleFunc("/patient-engagements", h.create).Methods(http.MethodPost)
	api.HandleFunc("/patient-engagements/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/patient-engagements/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/patient-engagements/{id}", h.delete).Methods(http.MethodDelete)
}
