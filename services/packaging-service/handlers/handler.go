...
import (
	...
	"github.com/klinova/kinara-os/packaging-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("packaging-service", nil))
	api.HandleFunc("/packagings", h.list).Methods(http.MethodGet)
	api.HandleFunc("/packagings", h.create).Methods(http.MethodPost)
	api.HandleFunc("/packagings/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/packagings/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/packagings/{id}", h.delete).Methods(http.MethodDelete)
}
