...
import (
	...
	"github.com/klinova/kinara-os/sanctions-check-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("sanctions-check-service", nil))
	api.HandleFunc("/sanctions-checks", h.list).Methods(http.MethodGet)
	api.HandleFunc("/sanctions-checks", h.create).Methods(http.MethodPost)
	api.HandleFunc("/sanctions-checks/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/sanctions-checks/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/sanctions-checks/{id}", h.delete).Methods(http.MethodDelete)
}
