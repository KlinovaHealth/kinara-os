...
import (
	...
	"github.com/klinova/kinara-os/reconciliation-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("reconciliation-service", nil))
	api.HandleFunc("/reconciliations", h.list).Methods(http.MethodGet)
	api.HandleFunc("/reconciliations", h.create).Methods(http.MethodPost)
	api.HandleFunc("/reconciliations/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/reconciliations/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/reconciliations/{id}", h.delete).Methods(http.MethodDelete)
}
