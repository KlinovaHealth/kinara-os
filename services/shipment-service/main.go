...
func main() {
	...
	queries := db.New(pool)
	shipmentHandler := handlers.NewShipmentHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"shipment-service"}`))
	...
	api.Use(pkgauth.RequireTenantScope("shipment-service", nil))
	shipmentHandler.RegisterRoutes(api)
	...
	go func() {
		log.Printf("shipment-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("shipment-service shutdown complete")
}
