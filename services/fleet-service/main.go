...
func main() {
	...
	queries := db.New(pool)
	fleetHandler := handlers.NewFleetHandler(queries)

	r := mux.NewRouter()
	r.Handle("/metrics", promhttp.Handler())
	r.HandleFunc("/health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"status":"ok","service":"fleet-service"}`))
	...
	api.Use(pkgauth.RequireTenantScope("fleet-service", nil))
	...
	go func() {
		log.Printf("fleet-service listening on :%s (TLS)", port)
	...
	defer cancel()
	srv.Shutdown(ctx)
	log.Println("fleet-service shutdown complete")
}
