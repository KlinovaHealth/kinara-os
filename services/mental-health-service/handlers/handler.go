...
import (
	...
	"github.com/klinova/kinara-os/mental-health-service/auth"
...
func (h *Handler) Register(r *mux.Router, jwtMW func(http.Handler) http.Handler) {
	...
	api.Use(jwtMW)
	api.Use(pkgauth.RequireTenantScope("mental-health-service", nil))
	api.HandleFunc("/mental-healths", h.list).Methods(http.MethodGet)
	api.HandleFunc("/mental-healths", h.create).Methods(http.MethodPost)
	api.HandleFunc("/mental-healths/{id}", h.get).Methods(http.MethodGet)
	api.HandleFunc("/mental-healths/{id}", h.update).Methods(http.MethodPut)
	api.HandleFunc("/mental-healths/{id}", h.delete).Methods(http.MethodDelete)
}
